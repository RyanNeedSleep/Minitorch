...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    # if a is too large, double may overflow
    # assume filter out some of that overflow problems
    assume(sigmoid(a) < 1 - 1e-6 and sigmoid(a) > 1e-6)
...
def test_symmetric(a: float, b: float) -> None:
    ...
    """
    # TODO: Implement for Task 0.2.
    return assert_close(mul(a, b), mul(b, a))
    # raise NotImplementedError("Need to implement for Task 0.2")
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    v1 = add(sum(ls1), sum(ls2))
    v2 = sum(addLists(ls1, ls2))
    assert_close(v1, v2)
